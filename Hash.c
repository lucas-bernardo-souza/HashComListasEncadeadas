...
//Tabela Hash usando lista encadeada como tratamento de colisões
...
// Tabela que é um vetor de ponteiros para listas
Lista *tabela[M];
...
// Criar lista vazia retornando seu endereço
Lista* criarLista(){
...
No* buscarNo(int mat, No *inicio){
    ...
    return NULL; //Caso a mátricula não for encontrada
}
...
int main() {
    ...
    do{
        ...
        case 1:
            ...
            break;
        case 2:
            printf("Qual a matrícula a ser buscada? ");
            scanf("%d", &mat);
            p = buscarPessoaTabela(mat);
            if(p) {
                printf("Pessoa encontrada: Matrícula: %d\tNome: %s", p->matricula, p->nome);
            }
            else
